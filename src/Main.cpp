...
PYBIND11_EMBEDDED_MODULE(engine_module, m) {
    ...
    m.def("get_window_width", []() { return WINDOW_WIDTH; }, "Get the window width");

    m.def("get_window_height", []() { return WINDOW_HEIGHT; }, "Get the window height");

    m.def(
        "multiply", [](int a, int b) { return a * b; }, "Multiply two numbers", py::arg("a"),
        py::arg("b"));
}
